...
const data_old = data_all.filter((f, idx) => idx < 1024)
const data_new = data_all.filter((f, idx) => idx >= 1024)
const data = data_old.slice();
...
data_new.forEach((dn) => {
    ...
    if (count_list.length === 0) {
        console.log(`result: ${dn[0]}, ${dn[1]}`)
        return;
    }

    console.log(`${dn[0]}, ${dn[1]} passed!`)
